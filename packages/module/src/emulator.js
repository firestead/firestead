...
export async function runEmulator(firesteadContext){
    //get config - project name
    let firebaseProject = firesteadContext.firebase?.config?.projectId || 'default'
    //on nuxt start -> maybe better with hook listen if supported
    const rootFBDir = `${firesteadContext._nuxt.rootDir}/_firestead/firebase`
