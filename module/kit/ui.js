...
export function getUINavigation(){
    ...
    return {
                ...
                createSidebar: () => {
                    const sidebarEntry = {
                        ...
                        createMenu: (sidebarName, sidebarPath, sidebarLabel) => {
                            ...
                            const filteredSidebarEntries = currentSidebar.items.filter(sidebarEntry => sidebarEntry.name === sidebarName)
                            const sidebarEntry = {
                                name: sidebarName,
                                path: sidebarPath,
                                label: sidebarLabel,
                            ...
                            if(filteredSidebarEntries.length===0){
                                currentSidebar.items.push(sidebarEntry)
                            }else{
                                //overwrite existing entry
                                currentSidebar.items[currentSidebar.items.indexOf(filteredSidebarEntries[0])] = sidebarEntry
                            }
                        },
                        getMenuItems: () => {
        ...
        getMenu: (name) => {
            const filteredEntries = firesteadContext.ui.navbar.filter(navEntry => navEntry.name === name)
